...
clientRouter.post('/api/clients/:clientId/locations', async (req: Request, res: Response) => {
  try {
    ...
    const locationId = new mongodb.ObjectId(); // Generate a new ObjectId for the location
...
clientRouter.post('/api/clients/:clientId/locations/:locationId/zones', async (req: Request, res: Response) => {
  try {
    ...
    const zoneId = new mongodb.ObjectId(); // Generate a new ObjectId for the zone
    const qrCodeUrl = await QRCode.toDataURL(zoneId.toHexString());
    ...
    if (result?.modifiedCount) {
      res.status(200).json({ message: 'Zone added successfully.', zone });
    } else {
      res.status(404).json({ message: 'Client or Location not found.' });
    }
  } catch (error: any) {
